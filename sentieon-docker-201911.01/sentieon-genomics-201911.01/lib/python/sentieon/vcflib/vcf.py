...
class VCF(sharder.Shardable):
    ...
    def __init__(self, path, mode='r'):
        ...
        if path.endswith('.gz'):
            self.fp = bgzf.open(path, self.mode)
            self.index = tabix.Tabix(path + '.tbi', self.mode)
        else:
            self.fp = io.open(path, self.mode)
            self.index = tribble.TribbleIndex(path + '.idx', self.mode)
        if self.mode[0:1] == 'r':
            self.load_header()
            self.parse_header()
        else:
    ...
    def __setstate__(self, ndict):
        ...
        mode = ndict['mode']
        if mode[0:1] == 'r':
            self.fp = (path.endswith('.gz') and bgzf.open(path, mode) or
                  io.open(path, mode))
    ...
    def sort_field(desc, alt, kv):
        ...
        if d['Number'] == '.':
            a = ['R'] + alt
            return (k, list(izip(*sorted(izip(a, v))))[1])
        if d['Number'] == 'A':
            return (k, list(izip(*sorted(izip(alt, v))))[1])
        if d['Number'] == 'G':
            a = ['R'] + alt
            ...
            if glsize == len(v):
                ...
                assert glsize == len(v)
                g = a
            return (k, list(izip(*sorted(izip(g, v))))[1])
        return kv
