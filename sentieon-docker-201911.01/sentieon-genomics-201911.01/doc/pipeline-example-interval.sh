...
fasta="/home/regression/references/b37/human_g1k_v37.fasta"
...
interval_file="/home/regression/references/b37/human_g1k_v37.target.interval_list"
...
workdir="$PWD/test/DNAseq_interval" #Determine where the output files will be stored
...
# 2a. Coverage metrics
# ******************************************
$SENTIEON_INSTALL_DIR/bin/sentieon driver -r $fasta -t $nt --interval $interval_file -i deduped.bam --algo CoverageMetrics coverage_metrics

# ******************************************
# 5. Base recalibration
# ******************************************
$SENTIEON_INSTALL_DIR/bin/sentieon driver -r $fasta -t $nt --interval $interval_file -i deduped.bam --algo QualCal -k $dbsnp -k $known_Mills_indels -k $known_1000G_indels recal_data.table
$SENTIEON_INSTALL_DIR/bin/sentieon driver -r $fasta -t $nt --interval $interval_file -i deduped.bam -q recal_data.table --algo QualCal -k $dbsnp -k $known_Mills_indels -k $known_1000G_indels recal_data.table.post
...
# ******************************************
$SENTIEON_INSTALL_DIR/bin/sentieon driver -r $fasta --interval $interval_file -t $nt -i deduped.bam -q recal_data.table --algo Haplotyper -d $dbsnp --emit_conf=30 --call_conf=30 output-hc.vcf.gz
